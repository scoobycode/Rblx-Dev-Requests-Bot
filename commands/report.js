...
let aaachannel = bot.channels.find(`id`, "411246419979141121")
	...
	let bmsgs = msgs.filter(m => m.embeds && m.embeds[0].fields && m.embeds[0].fields[4].value === message.author.id)
	let delmessage = bmsgs.first()
	if(delmessage) return message.reply("You cannot use this command because you already have a pending report. To prevent spam, you must wait until your report is accepted or denied.")
